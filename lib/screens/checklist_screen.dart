...
import '../widgets/duplicate_button.dart';
...
class _ChecklistScreenState extends State<ChecklistScreen> {
  ...
  Future<void> _duplicateItem(int index) async {
    final item = _checklistData.items[index];
    ...
    setState(() {
      ...
      _checklistData.items.insert(index + 1, duplicateItem);
    });
...
''';
  ...
  void _checkAndNext() async {
    ...
    // Find next unchecked item (search forward first, then loop back to beginning)
    int nextIndex = -1;
    ...
    if (nextIndex == -1) {
      for (int i = 0; i < _zoomedItemIndex; i++) {
        if (!_checklistData.items[i].isChecked) {
          nextIndex = i;
          break;
        }
    ...
    if (nextIndex != -1) {
      ...
      // No more unchecked items, exit zoom mode
      _exitZoomMode();
      // Show completion message
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('All items completed!'),
          backgroundColor: Colors.green,
        ),
      );
    }
    ...
    return Column(
      ...
      children: [
        ...
        ElevatedButton.icon(
          ...
          label: const Text('View Saved Checklists'),
          style: ElevatedButton.styleFrom(
                      ...
                      child: Card(
                        ...
                        child: ListTile(
                          ...
                          trailing: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              // Add duplicate button only for the first item
                              if (index == 0)
                                DuplicateButton(
